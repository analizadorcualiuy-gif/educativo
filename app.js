...
Entrevistado: Nos brinda herramientas increíbles para ahorrar tiempo, pero el verdadero motor sigue siendo el liderazgo empático. Una plataforma por sí sola no genera motivación; la motivación surge cuando las personas sienten que su trabajo tiene significado y son escuchadas a través de canales de comunicación fluidos.`,
    ...
    function removeCorpusExclusionsByTerm(term) {
        ...
        if (removedCount > 0) {
            recordAudit('Levantar exclusiones por término', `${removedCount} pasaje(s) del término "${term}" reincorporados.`);
            saveToStorage();
            if (state.activeDocId) setActiveDocument(state.activeDocId);
            renderCorpusExclusionList();
            renderDecoderList();
    ...
    function applyGlobalCorpusExclusion(termsString, scope = 'all', mode = 'turn') {
        ...
        recordAudit('Exclusión global de corpus', `Términos: "${rawTerms.join(', ')}" (${scopeLabel}). ${addedCount} pasaje(s) excluido(s).`);
        saveToStorage();
        if (state.activeDocId) {
            setActiveDocument(state.activeDocId);
        }
    ...
    function executeQuickExclusion() {
        ...
        if (scope === 'passage') {
            ...
            recordAudit('Exclusión manual de pasaje', `Fragmento excluido en doc ${docId}: "${finalText.slice(0, 40)}…"`);
            saveToStorage();
