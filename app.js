...
(function() {
    ...
    function projectLimits() {
        ...
        return {
            maxProjectChars: Math.floor(Math.max(256 * 1024 * 1024, Math.min(ramGiB * 128 * 1024 * 1024, 2 * 1024 * 1024 * 1024))),
            maxDocumentChars: Math.floor(Math.max(128 * 1024 * 1024, Math.min(ramGiB * 64 * 1024 * 1024, 1024 * 1024 * 1024))),
...
Entrevistado: Nos brinda herramientas increíbles para ahorrar tiempo, pero el verdadero motor sigue siendo el liderazgo empático. Una plataforma por sí sola no genera motivación; la motivación surge cuando las personas sienten que su trabajo tiene significado y son escuchadas a través de canales de comunicación fluidos.`,
    ...
    async function flushNativeSave() {
        ...
        if (pendingNativeProject) await flushNativeSave();
    }
    ...
    function setupNativeCloseFlush() {
        ...
        appWindow.onCloseRequested(async event => {
            if (nativeCloseInProgress || (!nativeSaveTimer && !pendingNativeProject && !nativeSaveInFlight)) return;
            event.preventDefault();
    ...
    async function generateCodedDocumentPDF() {
        ...
        if (window.PdfReportExporter && window.AnalyticsEngine) {
            ...
            await universalSaveFile(blob, `AnalizadorCualiUY_Pro_${doc.title.replace(/[^a-zA-Z0-9_\-]/g, '_')}_Codificado_${new Date().toISOString().slice(0, 10)}.pdf`);
            return;
        }
            ...
            <body>
                ...
                <div class="pdf-header">
                    ...
                    <div style="font-size:0.85rem; color:#64748b;">
                        ...
                        <strong>AnalizadorCualiUY Pro</strong> • <strong>Prof. Esp. Santiago Hernández</strong>
                    </div>
                </div>
...
${bodyHtml}
    ...
    function setupEventListeners() {
        ...
        if (licenseButton) {
            licenseButton.onclick = () => alert('Consulte EULA.txt, incluido con el producto, para leer las condiciones completas de la licencia comercial.');
        }
