...
// 1. Text of Jornadas IPES with normalized fluid paragraphs
const docIpesNormalized = `GRUPO FOCAL - LICEO 1 (JORNADAS IPES)
...
// 2. Updated categories for Jornadas IPES with rich search keywords
const ipesCategories = [
    [
        'cat-usos-eval',
        null,
        'CAT-USO',
...
// Helper to auto-generate initial codings for keywords so the project opens WITH MATCHES!
function findCodingsForKeywords(docId, docText, categories) {
    const codings = [];
    ...
    categories.forEach(cat => {
        ...
        keywords.forEach(kw => {
            ...
            while ((pos = normalizedText.indexOf(term, pos)) !== -1) {
                // Expand to sentence or turn
                let start = Math.max(0, docText.lastIndexOf('\n', pos));
                if (start > 0 && docText[start] === '\n') start++;
                let end = docText.indexOf('\n', pos + term.length);
                if (end === -1) end = docText.length;

                const quoteText = docText.slice(start, end).trim();
                if (quoteText.length > 15 && quoteText.length < 350) {
                    // Check duplicate overlap
                    const overlap = codings.some(c => c.docId === docId && c.categoryId === catId && Math.abs(c.startOffset - start) < 30);
                    if (!overlap) {
                        codings.push({
                            id: `code-${docId}-${catId}-${codings.length + 1}`,
                            docId,
                            categoryId: catId,
                            startOffset: start,
                            endOffset: end,
                            quoteText,
                            memo: `Coincidencia automática con término clave: "${kw}".`,
